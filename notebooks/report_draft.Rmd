...
state_comp <- data_filtered %>%
  filter(state == 'Connecticut' | state == 'Massachusetts') %>%
  ...
  geom_vline(xintercept = as.numeric(as.Date('2020-03-24')), color = "#00BFC4", size=.2, linetype = "dashed") +
  geom_vline(xintercept = as.numeric(as.Date('2020-05-18')), color = "#00BFC4", size=.2, linetype = "dashed") +
  annotate("text", x = as.Date('2020-04-24'), y = 54, label = "MA at-home-order", color = 'grey') +
  annotate("rect", xmin = as.Date('2020-03-24'), xmax = as.Date('2020-05-18'), ymin = -100, ymax = 50,
  alpha = .2, fill = "#00BFC4") +
  minimal_theme_RBGS +
  theme(axis.text.x = element_text(angle = 90)) +
  ggtitle('Connecticut vs. Massachusetts Mobility Trends')
          # , subtitle = "Comparing MA stay-at-home order to no stay-at-home order in CT")
```
